...
- **Stored procedures:** saved sequences of file operations, multiple sources, filename patterns, conditional steps, schedules, run history, and optional result notifications.
...
Stored procedures is available from Home, the sidebar and Settings. Save ordered copy, move, permanent delete, rename, create-folder, create-archive and extract-archive actions. Steps can be reordered or cloned and edited independently. Sources can be specific files or folders, or folder contents matched by `*` and `?`, optionally including subfolders. Each step resolves its locations when it runs, so later steps can use folders created by earlier ones. Destinations can create missing subfolders automatically. Archive passwords are not stored in procedures.

Enable **Ignore missing sources** on a step to continue with the sources that exist. Missing destinations and access or network failures still fail the step. If all sources are missing, the step succeeds with no completed items.

Names, child paths and source patterns accept `{date}` (`yyyy-MM-dd`), `{time}` (`HH-mm-ss`) and `{datetime}` (`yyyy-MM-dd_HH-mm-ss`). All steps use the same device-local date and time captured when the run starts. Double the braces for a literal token, such as `{{date}}`; other tokens stay as typed. For example, to move a folder's contents into a dated backup, select that folder with pattern `*` and kind **Files and folders**, use destination `/Backups/{datetime}`, and enable **Create missing folders**.

Each step can run unconditionally or when all or any of its conditions match earlier steps: succeeded, failed, skipped, completed items or no completed items. Conditions can reference any earlier step, and a conditional Stop action can end the run. Steps stop on failure by default; choose Continue to allow later steps to handle a failure. Completed changes remain, and a run with failures is still reported as failed even if later steps succeed. Item counts cover successful primary file actions, excluding automatic destination creation. A source pattern with no matches completes without changing anything; a step skipped by its conditions is reported separately.
